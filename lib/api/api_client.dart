...
class ApiClient extends GetxService {
  ...
  late Map<String, String> _secondaryHeaders;
  ...
  ApiClient({required this.appBaseUrl, required this.sharedPreferences}) {
    ...
    try {
      // log("In ApiClint Constructor:::AppConstants.userAddress::::${AppConstants.userAddress}");
      addressModel = AddressModel.fromJson(
          jsonDecode(sharedPreferences.getString(AppConstants.userAddress)!));
    } catch (_) {
      // log("After first exception:::In ApiClint Constructor");
    }
    int? moduleID;
    ...
    _secondaryHeaders = updateHeader2(
      token,
      addressModel?.zoneIds,
      addressModel?.latitude,
      addressModel?.longitude,
  ...
  Future<Response> getData(String uri,
    ...
    try {
      ...
      // log("API_CLIEND:::(headers)::::$headers");
      // log("API_CLIEND:::(_mainHeaders)::::$_mainHeaders");
  ...
  Future<MediaModel?> getDetailsList({int? userID}) async {
    userID = 3;
    final url =
  ...
  Future<Response> postData(String uri, dynamic body,
    ...
    try {
      ...
      // log("API_CLIEND:::response::$response");
      // log("API_CLIEND:::response.body::${response.body}");
      return handleResponse(response, uri, handleError);
    } catch (e) {
      return Response(statusCode: 1, statusText: noInternetMessage);
    }
  ...
  Response handleResponse(
    ...
    dynamic body;
    try {
      // log("In API TRY BLOCK:::");
      // log("In API TRY BLOCK:::uri::::$uri");
      // log("In API TRY BLOCK:::uri::::$uri");
