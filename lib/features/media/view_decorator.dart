...
//
//     if (response != null) {
//       if (response['user_details'] != null && decoratorProfile == null) {
//         decoratorProfile = response['user_details'];
//         String aboutUs = decoratorProfile?['about_us'] ?? '';
//       }
//
...
//                           ],
//                         ),
//                         if (decoratorProfile!['about_us'] != null && decoratorProfile!['about_us'].toString().trim().isNotEmpty)
//                           Padding(
...
//
//
// // Add this class definition to your code (can be placed at the bottom of the file)
...
class _ViewDecoratorState extends State<ViewDecorator> {
  ...
  List<int> wishlistMediaIds = [];
  String? errorMessage;
  List<int> eventsWithMedia = []; // New list to store events that have media
  ...
  void fetchEventList() async {
    events = await apiService.fetchEvents();
    if (events != null && events!.isNotEmpty) {
      // First fetch all events, then filter those with media
      eventIds = events!.map((event) => event['id'] as int).toList();
      eventNames = events!.map((event) => event['title'] as String).toList();
